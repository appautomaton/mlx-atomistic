...
- A secondary `atomistic_prep` package for prepared-artifact and ligand-receptor workflow CLI operations (`pyproject.toml`, `src/atomistic_prep/cli.py`).
...
| Preparation CLI | `src/atomistic_prep/` | Build/import prepared systems and run short MLX workflows | `uv run atomistic-prep ...` | Declared as `atomistic-prep = atomistic_prep.cli:main` in `pyproject.toml`. |
| Benchmark CLI | `src/mlx_atomistic/benchmarks/` | MD and DFT benchmark/validation entry points | `uv run mlx-atomistic-benchmark ...`; module benchmarks | Declared as `mlx-atomistic-benchmark = mlx_atomistic.benchmarks.md_performance:main` in `pyproject.toml`; examples are listed in `README.md`. |
...
- Wheel packages are `src/mlx_atomistic` and `src/atomistic_prep` (`pyproject.toml`).
- Source code should stay under `src/mlx_atomistic/` unless a task explicitly targets the `atomistic_prep` package (`AGENTS.md`, `pyproject.toml`).
...
- Whether `atomistic_prep` should remain a peer package in this repository or eventually fold behind `mlx_atomistic` workflow APIs (`pyproject.toml`, `src/atomistic_prep/cli.py`).
...
- Console scripts declared in `pyproject.toml` are the stable CLI surfaces: `atomistic-prep` and `mlx-atomistic-benchmark`.
...
- Preparation CLI production boundaries and fail-closed behavior around prepared artifacts (`src/atomistic_prep/cli.py`).
...
- `src/atomistic_prep/cli.py` - preparation CLI subcommands and workflow surface.
