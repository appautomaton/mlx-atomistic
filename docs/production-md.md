...
For Metal fixed-cell production runs, `mlx_interaction32` is selected only in
the independently measured 23,000--31,000 and 90,000--100,000 atom windows,
with order-5 PME, 9 A cutoff, 5.5 A skin, an orthorhombic cell, one compatible
PME force term, and no atom-pair NBFIX. CHARMM type-pair NBFIX is supported.
Other PME workloads retain exact tiles or compact pairs. Checkpoint resume pins
the recorded neighbor backend and fails closed if that backend is no longer
admissible. The force, trajectory, memory, constraint, and multi-rebuild
promotion evidence is recorded in
[`md-acceleration.md`](./md-acceleration.md).
