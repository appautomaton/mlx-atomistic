...
faithfully. PME and first-path NPT support remain bounded surfaces: accepted
artifacts must provide explicit configuration and readiness metadata, and
unsupported production cases remain blockers. The neutral orthorhombic PME
subset has the validated envelope described below; first-path NPT remains
proof-level.

Neutral orthorhombic PME now has a validated production envelope through
24,488 atoms on the deterministic TIP3P/0.15 M NaCl target. Fixed-coordinate
OpenMM parity, short NVE/NVT stability, compact-neighbor execution, timing, and
memory evidence are recorded in
[the M5 Max report](./benchmarks/dhfr-scale-neutral-pme-validation-m5max.md).
Charged systems, triclinic PME, analytic PME virial/NPT, and protein/membrane
production remain outside that envelope.
...
- Neutral orthorhombic PME is validated through 24,488 atoms on a controlled
  electrolyte fixture. This advances the electrostatics scale boundary but does
  not cover the 92,001-atom membrane fixture, its net-charge convention, or its
  triclinic/pressure requirements.
...
SMD. Neutral orthorhombic PME has a bounded validated 24,488-atom surface; NPT
and the remaining PME conventions are not evidence for a complete
membrane-production workflow.

For the GPCRmd 729 production-readiness probe, the next implementation blocker
is PME-scale electrostatics beyond the validated neutral orthorhombic envelope.
The neighbor-listed real-space path is validated at 92,001 synthetic atoms and
PME is validated at 24,488 neutral orthorhombic atoms, but the real GPCRmd
fixture still needs a local-cache rerun and remains outside the PME atom-count,
charge-policy, triclinic, and analytic-virial/NPT boundaries. Production-MD
readiness therefore remains blocked for the selected membrane system.
