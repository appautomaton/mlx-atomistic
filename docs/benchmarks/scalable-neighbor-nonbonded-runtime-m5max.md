...
through 92,001 atoms on a synthetic orthorhombic system. It does not claim a
GPCRmd production run, PME-at-scale support, or an OpenMM/LAMMPS performance
ratio.
...
The neighbor/topology axis is now proven at the target atom count. The next
production blocker for the selected GPCRmd membrane fixture is PME-scale
electrostatics: the current production readiness envelope is capped at 4,096
atoms, with net-charge policy, analytic PME virial, and triclinic PME still
deferred.
