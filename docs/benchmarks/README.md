...
| [md-left-grouped-neighbor-scatter-m5max.md](./md-left-grouped-neighbor-scatter-m5max.md) | mlx_atomistic | Adaptive left-grouped exact-tile scatter on 5DFR, JAC, and GPCRmd | Metal | Apple M5 Max |
...
| [same-workload-dhfr-stretch.md](./same-workload-dhfr-stretch.md) | mlx_atomistic/openmm-reference | DHFR preparation status | Metal/OpenCL where available | local |
...
| `uv run python scripts/benchmark_small_constraint_clusters.py results/scalable-charged-pme-runtime/jac-2x2x1/prepared --out results/interaction-engine-v2/small-constraints-jac.json` | mlx_atomistic | opt-in performance | interleaved generic-versus-Metal constraint timing and parity under `results/` |
...
| `uv run python -m mlx_atomistic.benchmarks.dhfr --case dhfr-amber20-jac-pme --steps 1 --amber-topology results/inputs/Amber20_Benchmark_Suite/PME/Topologies/JAC.prmtop --amber-coordinates results/inputs/Amber20_Benchmark_Suite/PME/Coordinates/JAC.inpcrd --json > results/scalable-charged-pme-runtime/jac-1x/runtime-smoke.json` | mlx_atomistic | opt-in AMBER20 JAC DHFR charged-PME smoke | normalized one-step JSON; no cross-engine ratio without a matching runtime manifest |
...
| `uv run python scripts/benchmark_openmm_dhfr.py --case dhfr-amber20-jac-pme --platform Reference --steps 1 --json > results/same-workload-openmm-comparison/openmm-dhfr-amber20-jac-pme.json` | openmm-reference | opt-in AMBER20 JAC DHFR shape check | reference-only JSON; not a charged-parity or throughput comparator without a matching manifest |
