...
| `src/mlx_atomistic/benchmarks/pme_fixture.py` | Deterministic neutral water/salt fixture family, including the 24,488-atom target with stable content hash and prepared-artifact output. | `tests/test_pme_validation.py`; [dhfr-scale-neutral-pme-validation-m5max.md](./dhfr-scale-neutral-pme-validation-m5max.md). | Generated artifacts under `results/dhfr-scale-neutral-pme-validation/`. | Fast fixture smoke; opt-in at target scale. | Controlled orthorhombic TIP3P/NaCl fixture, not protein or membrane preparation. |
| `src/mlx_atomistic/benchmarks/pme_validation.py` | Converged direct-Ewald checks on small fixtures and matched-parameter OpenMM parity through 24,488 atoms. | `tests/test_pme_validation.py`, `tests/test_openmm_mlx_parity.py`; target report above. | Ewald JSON and MLX/OpenMM parity artifacts under `results/dhfr-scale-neutral-pme-validation/`. | Fast classification tests; opt-in reference/Metal runs. | Validates neutral orthorhombic PME only; charged and triclinic conventions remain deferred. |
| `src/mlx_atomistic/benchmarks/pme_stability.py` | Bounded constrained minimization plus 1 ps NVE timestep convergence and NVT temperature/constraint evidence on the target. | `tests/test_pme_validation.py`; target report above. | `results/dhfr-scale-neutral-pme-validation/stability/`. | Opt-in Metal dynamics. | The deterministic lattice is not an equilibrated liquid; report retains the NVE heating caveat. |
| `src/mlx_atomistic/benchmarks/pme_performance.py` | Target-safe PME/FFT/direct/correction/production timing, RSS/MLX memory sampling, provenance linkage, and fail-closed same-workload comparison. | `tests/test_benchmarks.py` profiler, memory-growth, and comparison coverage; target report above. | `results/dhfr-scale-neutral-pme-validation/profile/pme-profile.json`. | Opt-in performance; blocked-path smoke is fast. | Requires `mlx_cell_pairs`; the dense quadratic oracle is explicitly skipped above 4,096 atoms. No OpenMM ratio without matching operation, precision, and timing metric. |
