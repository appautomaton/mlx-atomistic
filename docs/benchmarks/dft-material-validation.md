...
| 2H-Si variable-cell admission, 4 atoms, PBE/GTH-q4, 25 Ha, 6×6×4 | The cell started at `0.995` of the cubic-derived ideal 2H scale. One base SCF converged, but frozen stress at the primary and doubled strain differed by `6.72416e-4 Ha/bohr³`; the workflow stopped with `stress_failed`, 0 accepted steps, and 11.956 s wall time. | The deterministic stress, cell-only, coupled, and checkpoint oracles pass. The source-bound material trajectory does not. | Current-verified negative evidence on Apple M5 Max, AC power, normal power mode. Thresholds were not weakened. Analytic or otherwise Pulay-aware stress is required before variable-cell material verification. |
...
chemistry, pseudopotential transferability, or variable-cell relaxation. The
failed 2H-Si admission is retained as a boundary, not promoted to a verified
result. These rows do not establish universal periodic DFT accuracy.
