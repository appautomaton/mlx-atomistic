...
| Rock-salt MgO primitive Mg-q10/O-q6 feasibility, 2 atoms, 40 Ha, historical symmetry-reduced 4×4×4 | Complete wall `40.989 s`; peak temporary memory `57.2 MB`; 80 SCF iterations | Numerical diagnostic only; no EOS reference claim | Rejected: maximum orbital residual `4.329e-6` exceeded the locked `2e-6` gate, and orbit-weight aggregation did not reconstruct rotated SCF densities. Future candidates use a full mesh with time reversal only. No full q10 EOS was run. |
...
| bcc Fe primitive cell, spin-PBE/GTH-q16, Fermi-Dirac `0.01 Ha`, unfolded 4×4×4, 10 bands | The reduced result gives `2.41795 μB/atom` in `8.056 s`; the matched full mesh gives `2.33818 μB/atom` in `49.310 s`. | Published PBE context `2.33 μB/atom`; the full-mesh moment error is `0.00818 μB/atom`. | Current-verified numerical oracle on Apple M5 Max, AC power, normal mode. Reduction changes free energy by `0.005622 Ha/atom` and moment by `0.07977 μB/atom`, failing the locked `5e-5` and `0.02` gates. Reduced cutoff/k-point checks are not promoted to full-mesh transferability evidence. |
