...
def test_dense_disjoint_composite_matches_sequential_constraint_oracle(monkeypatch):
    ...
    cell = Cell.cubic(12.0)
    assert constraints._dense_affine_velocity_step_ready(
        reference,
        velocities,
        masses,
        cell,
    )
    assert not constraints._dense_affine_velocity_step_ready(
        reference.astype(mx.float16),
        velocities,
        masses,
        cell,
    )
    ...
    assert not constraints._dense_affine_velocity_step_ready(
        ...
        masses,
        Cell.triclinic(
            [[12.0, 0.5, 0.0], [0.0, 12.0, 0.0], [0.0, 0.0, 12.0]]
        ),
    )
    dense_calls = []
...
@pytest.mark.gpu
def test_dense_affine_constraint_velocities_match_full_periodic_graph(monkeypatch):
    """Affine producer loads preserve periodic corrections, kicks, and free atoms."""
    ...
    velocities = mx.array(
        np.random.default_rng(81).uniform(-0.2, 0.2, size=positions.shape),
        dtype=mx.float32,
    )
    forces = mx.array(
        np.random.default_rng(82).uniform(-0.4, 0.4, size=positions.shape),
        dtype=mx.float32,
    )
    masses = mx.array(
        [12.0, 14.0, 1.0, 12.0, 1.0, 16.0, 10.0, 1.0, 1.0, 1.0, 9.0, 1.0],
    ...
    cell = Cell.cubic(12.0)
    offsets = mx.array(
        np.random.default_rng(83).uniform(-0.004, 0.004, size=positions.shape),
        dtype=mx.float32,
    )
    offsets = offsets.at[0, 0].add(12.0).at[6, 1].add(-12.0)
    predicted_positions = positions - offsets
    time_step = 0.01
    position_correction = cell.minimum_image(positions - predicted_positions)
    explicit_pre_base = velocities + position_correction / time_step
    expected_pre = constraints._apply_pre_force_velocities(
        positions,
        explicit_pre_base,
        masses,
        cell,
    ...
    force_scale = 1.75
    half_time_step = 0.003
    acceleration = force_scale * forces / masses[:, None]
    explicit_final_base = expected_pre + half_time_step * acceleration
    expected_final = constraints.apply_velocities(
        positions,
        explicit_final_base,
        masses,
        cell,
    ...
    calls = {"dense": 0, "settle": 0, "shake": 0}
    original_dense = constraints_module._dense_constraint_apply
    original_settle = constraints_module._settle_water_velocity_deltas
    original_shake = constraints_module._shake_cluster_velocity_deltas
    ...
    actual_pre = constraints._apply_pre_force_affine_velocities(
        positions,
        predicted_positions,
        velocities,
        masses,
    ...
    actual_final = constraints._apply_final_affine_velocities(
        positions,
        expected_pre,
        forces,
        masses,
    ...
    mx.eval(expected_pre, actual_pre, expected_final, actual_final)

    assert calls == {"dense": 2, "settle": 1, "shake": 2}
    np.testing.assert_allclose(
        np.asarray(actual_pre),
        np.asarray(expected_pre),
        rtol=1.0e-6,
        atol=2.0e-6,
    )
    np.testing.assert_allclose(
    ...
    free_atoms = [1, 6, 10]
    np.testing.assert_allclose(
        np.asarray(actual_pre)[free_atoms],
        np.asarray(explicit_pre_base)[free_atoms],
        rtol=0.0,
        atol=2.0e-6,
    )
    np.testing.assert_allclose(
        np.asarray(actual_final)[free_atoms],
        np.asarray(explicit_final_base)[free_atoms],
        rtol=0.0,
        atol=2.0e-6,
    )
...
def test_dense_composite_declines_overlap_and_runtime_profiling(monkeypatch):
    ...
    affine_calls = {"final": 0, "pre": 0}

    def fail_affine_pre(*args, **kwargs):
        affine_calls["pre"] += 1
        raise AssertionError("profiled path must not invoke affine constraints")

    def fail_affine_final(*args, **kwargs):
        affine_calls["final"] += 1
        raise AssertionError("profiled path must not invoke affine constraints")
    ...
    monkeypatch.setattr(
        CompositeConstraints,
        "_apply_final_affine_velocities",
        fail_affine_final,
    )
    ...
    assert affine_calls == {"final": 0, "pre": 0}


@pytest.mark.gpu
def test_dense_affine_constraints_preserve_seeded_short_trajectory(monkeypatch):
    """The admitted affine path matches the retained full-graph trajectory."""
    ...
    velocities = np.random.default_rng(84).uniform(
        -0.02,
        0.02,
        size=positions.shape,
    ).astype(np.float32)
    masses = np.asarray([16.0, 1.0, 1.0, 12.0, 1.0, 1.0, 10.0, 11.0], dtype=np.float32)
    ...
    def run():
        ...
        potential = NonbondedPotential(
            sigma=np.full((positions.shape[0],), 0.2, dtype=np.float32),
            epsilon=np.full((positions.shape[0],), 0.01, dtype=np.float32),
    ...
    original_gate = md_module._dense_affine_constraint_candidate_enabled
    admissions = []
    affine_calls = {"final": 0, "pre": 0}
    original_pre = CompositeConstraints._apply_pre_force_affine_velocities
    original_final = CompositeConstraints._apply_final_affine_velocities

    def record_gate(**kwargs):
        admitted = original_gate(**kwargs)
        admissions.append(admitted)
        return admitted

    def record_pre(self, *args, **kwargs):
        affine_calls["pre"] += 1
        return original_pre(self, *args, **kwargs)

    def record_final(self, *args, **kwargs):
        affine_calls["final"] += 1
    ...
    affine = run()
    assert any(admissions)
    assert affine_calls == {"final": 3, "pre": 3}
    monkeypatch.setattr(
        md_module,
        "_dense_affine_constraint_candidate_enabled",
        lambda **kwargs: False,
    ...
    explicit = run()
    mx.eval(
        affine.final_state.positions,
        affine.final_state.velocities,
        affine.final_state.forces,
        explicit.final_state.positions,
        explicit.final_state.velocities,
        explicit.final_state.forces,
    )

    for actual, expected in (
        (affine.final_state.positions, explicit.final_state.positions),
        (affine.final_state.velocities, explicit.final_state.velocities),
        (affine.final_state.forces, explicit.final_state.forces),
        (affine.constraint_max_error, explicit.constraint_max_error),
    ):
        np.testing.assert_allclose(
            np.asarray(actual),
            np.asarray(expected),
