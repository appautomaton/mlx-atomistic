...
def _staged_compile_nvt_fixture():
    positions = np.asarray(
        [
            [1.0, 1.0, 1.0],
            [2.0, 1.0, 1.0],
    ...
    velocities = np.tile(
        np.asarray([[2.0, 0.1, -0.05]], dtype=np.float32),
        (positions.shape[0], 1),
    )
    masses = np.asarray([16.0, 1.0, 1.0, 12.0, 1.0, 1.0, 1.0], dtype=np.float32)
    cell = Cell.cubic(8.0)
...
@pytest.mark.gpu
def test_staged_compiled_nvt_matches_eager_and_reuses_callables(monkeypatch):
    """Staged compilation preserves the production constraint trajectory."""

    original_compile = mx.compile
    compiled_names = []

    def counted_compile(function, *args, **kwargs):
        compiled_names.append(function.__name__)
        return original_compile(function, *args, **kwargs)

    monkeypatch.setattr(mx, "compile", counted_compile)
    compiled, compiled_manager = _staged_compile_nvt_fixture()
    monkeypatch.setattr(
        md_module,
        "_staged_integration_compile_enabled",
        lambda *args, **kwargs: False,
    )
    eager, eager_manager = _staged_compile_nvt_fixture()
    mx.eval(
        compiled.final_state.positions,
        compiled.final_state.velocities,
        compiled.final_state.forces,
        compiled.potential_energy,
        compiled.constraint_max_error,
        eager.final_state.positions,
        eager.final_state.velocities,
        eager.final_state.forces,
        eager.potential_energy,
        eager.constraint_max_error,
    )

    assert compiled_names.count("staged_pre_force_step") == 1
    assert compiled_names.count("staged_post_force_step") == 1
    assert compiled_manager.rebuild_count > 1
    assert compiled_manager.rebuild_count == eager_manager.rebuild_count
    for compiled_value, eager_value, tolerance in (
        (compiled.final_state.positions, eager.final_state.positions, 2.0e-6),
        (compiled.final_state.velocities, eager.final_state.velocities, 2.0e-6),
        (compiled.final_state.forces, eager.final_state.forces, 3.0e-5),
        (compiled.potential_energy, eager.potential_energy, 2.0e-5),
        (compiled.constraint_max_error, eager.constraint_max_error, 2.0e-6),
        (compiled.pair_count, eager.pair_count, 0.0),
        (compiled.rebuild_count, eager.rebuild_count, 0.0),
    ):
        np.testing.assert_allclose(
            np.asarray(compiled_value),
    ...
    assert {
        key: value
        for key, value in compiled.runtime_sync_report.items()
        if key.endswith("_count")
    } == {
        key: value
        for key, value in eager.runtime_sync_report.items()
        if key.endswith("_count")
    }


@pytest.mark.gpu
def test_staged_compile_preserves_async_force_boundary_across_rebuild(monkeypatch):
    """Compiled calls remain staged around neighbor admission and force submit."""

    events = []
    original_compile = mx.compile
    original_async_eval = mx.async_eval
    original_update = NeighborListManager.update

    def ordered_compile(function, *args, **kwargs):
        compiled = original_compile(function, *args, **kwargs)
        if function.__name__ not in {
            "staged_pre_force_step",
            "staged_post_force_step",
        }:
            return compiled

        def ordered_call(*call_args, **call_kwargs):
            events.append(function.__name__)
            return compiled(*call_args, **call_kwargs)

        return ordered_call

    def ordered_async_eval(*values):
        events.append("async_eval")
        return original_async_eval(*values)

    def ordered_update(self, positions):
        events.append("neighbor_update")
        return original_update(self, positions)

    monkeypatch.setattr(mx, "compile", ordered_compile)
    monkeypatch.setattr(mx, "async_eval", ordered_async_eval)
    monkeypatch.setattr(NeighborListManager, "update", ordered_update)
    result, manager = _staged_compile_nvt_fixture()
    mx.eval(result.final_state.positions, result.final_state.velocities)

    pre_indices = [
        index
        for index, event in enumerate(events)
        if event == "staged_pre_force_step"
    ]
    post_indices = [
        index
        for index, event in enumerate(events)
        if event == "staged_post_force_step"
    ]
    async_indices = [
        index for index, event in enumerate(events) if event == "async_eval"
    ]
    assert len(pre_indices) == len(post_indices) == len(async_indices) == 5
    for pre_index, async_index, post_index in zip(
        pre_indices,
        async_indices,
        post_indices,
        strict=True,
    ):
        assert pre_index < async_index < post_index
        assert "neighbor_update" in events[pre_index + 1 : async_index]
