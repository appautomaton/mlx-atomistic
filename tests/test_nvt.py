...
def test_langevin_step_units_match_inline_constraint_arithmetic():
    positions = mx.array(
    ...
    force_scale = 0.75
    velocity_decay = 0.91
    thermal_scale = mx.array([[0.02], [0.03]], dtype=mx.float32)

    acceleration = force_scale * forces / masses[:, None]
    velocity_half = velocities + 0.5 * dt * acceleration
    predicted = positions + 0.5 * dt * velocity_half
    keys = mx.random.split(key, 2)
    noise = mx.random.normal(velocities.shape, key=keys[1])
    middle = velocity_decay * velocity_half + thermal_scale * noise
    predicted = cell.wrap(predicted + 0.5 * dt * middle)
    ...
    correction = cell.minimum_image(expected_positions - predicted)
    expected_pre_velocity = constraints.apply_velocities(
        expected_positions,
        middle + correction / dt,
        masses,
        cell,
    )

    actual_positions, actual_pre_velocity, actual_key = _langevin_pre_force_step(
        positions,
        velocities,
        forces,
        key,
        masses,
        dt=dt,
        force_to_acceleration_scale=force_scale,
        velocity_decay=velocity_decay,
        thermal_scale=thermal_scale,
        cell=cell,
        wrap_positions=True,
        constraints=constraints,
        pre_force_velocity_projector=constraints.apply_velocities,
    )
    next_forces = forces * 0.8
    expected_post_velocity = constraints.apply_velocities(
        expected_positions,
        expected_pre_velocity
        + 0.5 * dt * force_scale * next_forces / masses[:, None],
        masses,
        cell,
    )
    actual_post_velocity = _langevin_post_force_step(
        actual_positions,
        actual_pre_velocity,
        next_forces,
        masses,
    ...
    mx.eval(
        ...
        actual_positions,
        actual_pre_velocity,
        actual_post_velocity,
        actual_key,
    )

    np.testing.assert_array_equal(np.asarray(actual_key), np.asarray(keys[0]))
    np.testing.assert_allclose(actual_positions, expected_positions, rtol=0.0, atol=1e-7)
...
def test_ordinary_constrained_langevin_uses_step_units_between_validation_steps(
    monkeypatch,
):
    positions, velocities, cell, potential = _small_system()
    constraints = DistanceConstraints([(0, 1)], distances=[1.2])
    calls = {"pre": 0, "post": 0}
    original_pre = md_module._langevin_pre_force_step
    original_post = md_module._langevin_post_force_step
    ...
    monkeypatch.setattr(md_module, "_langevin_pre_force_step", counted_pre)
    monkeypatch.setattr(md_module, "_langevin_post_force_step", counted_post)
    result = simulate_nvt(
        positions,
        velocities,
