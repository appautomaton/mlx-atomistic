---
title: "Same-Workload DHFR Preparation Status"
---
...
Pair ids: `dhfr-implicit`, `dhfr-amber20-jac-pme`.
...
`dhfr-amber20-jac-pme` now runs under the explicit
...
`JAC` is the AMBER20 benchmark-suite file stem for DHFR. It is not a second
protein. The AMBER20 JAC and OpenMM 5DFR inputs contain the same 159-residue
DHFR protein and 7,023 waters, but they are different MD preparations and
cannot substitute for each other in fixed-coordinate comparisons:

| Preparation | Source | Preparation boundary |
| --- | --- | --- |
| `dhfr-amber20-jac-pme` | AMBER20 `JAC.prmtop` + `JAC.inpcrd` | Parameters and coordinates are imported from the AMBER files |
| `dhfr-5dfr-pme` | OpenMM `5dfr_solv-cube_equil.pdb` + Amber99SB/TIP3P | OpenMM constructs and provenance-locks a separate force system |
...
| DHFR OpenMM 5DFR Explicit PME | 23.6k atoms | 752.5 ns/day |

The 752.5 ns/day row uses the OpenMM 5DFR preparation, not AMBER20 JAC. It is
useful hardware context but is excluded from the AMBER20 JAC throughput
comparison. The fixed-coordinate JAC parity run constructs JAC independently
in OpenMM and is the correct reference for that preparation.
...
- `dhfr-amber20-jac-pme`: runnable with the explicit
